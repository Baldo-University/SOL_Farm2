...
static void *sighandler(void *arg) {
	...
	for(;;) {
		...
		if(r!=0) {
			errno=r;
			DEBUGGER(perror("ERRORE FATALE 'sigwait'"));
...
void masterworker(int argc, char *argv[], char *socket) {
	...
	*/
	...
	while((opt=getopt(argc,argv,"n:q:d:t:"))!=-1) {
		...
		case 'n':
			if(!(optarg && *optarg)) {
				DEBUGGER(fprintf(stderr,"Errore nella lettura dell'opzione -%c.\n",opt));
				break;
			}
			...
			if(errno==ERANGE || optlong<MIN_THREADS)	//di default, MIN_THREADS e' 1
				DEBUGGER(fprintf(stderr,"Ignorata opzione -%c non valida (numero di thread worker).\n",opt));
			else
				workers=optlong;
			break;
		...
		case 'q':
			...
			if(errno==ERANGE || optlong<=0)
				DEBUGGER(fprintf(stderr,"Ignorata opzione -%c non valida (dimensione coda di produzione).\n",opt));
			else
		...
		case 'd':
			...
			if(strlen(optarg)>MAX_NAMELENGTH) {
				DEBUGGER(fprintf(stderr,"Opzione -%c, pathname troppo lungo scartato.\n",opt));
				break;
			}
			...
			if(!S_ISDIR(info.st_mode)) {
				DEBUGGER(fprintf(stderr,"Opzione -%c, pathname indicato %s non e' directory.\n",opt,optarg));
				break;
			}
		...
		case 't':
			...
			optlong=strtol(optarg,NULL,10);
			if(errno==ERANGE || optlong<0)
				DEBUGGER(fprintf(stderr,"Ignorata opzione -%c non valida (attesa di inserimento in coda di produzione).\n",opt));
			else
				queue_delay=optlong;
			break;
		default:
			DEBUGGER(fprintf(stderr,"Passata opzione -%c non riconosciuta.\n",opt));
		}
	}
	DEBUGGER(fprintf(stderr,"Masterworker: getopt completata\n"));
	DEBUGGER(fprintf(stderr,"dim. iniziale threadpool: %ld\nlungh. coda: %ld\nritardo inserimento: %ld secondi\n", workers,queue_length,queue_delay));

	/*creazione threadpool*/
	threadpool_t *pool=initialize_pool(workers,queue_length,socket);
	ec_is(pool,NULL,"masterworker, initialize_pool");
	DEBUGGER(fprintf(stderr,"Masterworker: threadpool inizializzato\n"));
	...
	for(i=optind;i<argc;i++) {
		...
		while(thread_num_change!=0) {
			if(thread_num_change>0) {
				DEBUGGER(fprintf(stderr,"Master: ricevuto SIGUSR1\n"));
				add_worker(pool);
				thread_num_change--;
			}
			else if(thread_num_change<0) {
				DEBUGGER(fprintf(stderr,"Master: ricevuto SIGUSR2\n"));
				remove_worker(pool);
				thread_num_change++;
			}
		}
		...
		if(check_stat==-1) {
			DEBUGGER(perror("Masterworker, stat di file regolare"));
			continue;
		}
		if(!S_ISREG(info.st_mode))
			DEBUGGER(fprintf(stderr,"Passato file %s non regolare da linea di comando, scartato.\n",argv[i]));
			...
			if(res)	//invio file
				DEBUGGER(fprintf(stderr,"Filefinder: argv[%d], enqueue fallita con errore %d",i,res));
	...
	while(directories!=NULL) {
		...
		if(dir==NULL) {
			DEBUGGER(fprintf(stderr,"Directory %s non trovata\n",directories->name));
			continue;
		}
		struct dirent *file;
		while((errno=0, file=readdir(dir))!=NULL) {
			//controllo errno
			if(errno!=0) {
				DEBUGGER(fprintf(stderr,"Errore nell'apertura di %s\n",directories->name));
			...
			if(!strncmp(file->d_name,".",2) || !strncmp(file->d_name,"..",3))	//ignora se stessa e la dir padre
				...
				DEBUGGER(fprintf(stderr,"trovata directory %s\n",file->d_name));
				...
				DEBUGGER(fprintf(stderr,"Filefinder: inserisco file %s\n",fullpathname));
			}
			
			else if(file->d_type==DT_UNKNOWN)
				DEBUGGER(perror("masterworker, file di tipo sconosciuto"));
		}
	...
	DEBUGGER(fprintf(stderr,"masterworker: filefinder termina\n"));
	
	/*master non inserisce piu' file in coda e attende che il threadpool termini*/
	long finalworkers=await_pool_completion(pool);
	if(finalworkers<1)
		DEBUGGER(fprintf(stderr,"masterworker, await_pool_completion resituisce numero non valido di thread\n"));
	else {
		FILE *workers_file=fopen("nworkeratexit.txt","w");
		if(workers_file==NULL)
			DEBUGGER(perror("masterworker, fopen nworkeratexit"));
		else {
			fprintf(workers_file,"%ld\n",finalworkers);	//inserisci il numero di thread alla fine
			DEBUGGER(fprintf(stderr,"Masterworker: ricevuti %ld worker\n",finalworkers));
			fclose(workers_file);
		}
	}
	DEBUGGER(fprintf(stderr,"Masterworker: threadpool ha concluso il suo lavoro\n"));
	...
	DEBUGGER(fprintf(stderr,"Masterworker: chiusura\n"));
}
